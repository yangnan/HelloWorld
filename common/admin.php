...
include_once('require/db_mysql.php');
//数据库连接

$db = new DB(db_config::$dbconfig["db_server"], db_config::$dbconfig["db_user"], db_config::$dbconfig["db_pwd"], __DATANAME__, $pconnect);
...
function GetLang($lang,$EXT="php"){
	...
	if($lang == 'email' || $lang == 'log' || $lang == 'bbscode'){
		$path="language/sqlbaklanguage/cp_lang_$lang.$EXT";
		!file_exists($path) && $path="language/sqlbaklanguage/cp_lang_$lang.$EXT";
		return $path;
	} else{
		$L_P =  eregi('^c_',$lang) ? C_P : R_P;
		$adminpath=file_exists("language/sqlbaklanguage/cp_lang_$lang.$EXT") ? "admin_$tplpath" : 'admin';		$path="language/sqlbaklanguage/cp_lang_$lang.$EXT";
	}
	return $path;
}
...
function PrintEot($template,$EXT="htm")
{
	...
	$L_P =  "templates/admin/common/".$template;
	///cms
	...
	///cms
	$adminpath=file_exists($L_P.".$EXT") ? "admin_$tplpath" : 'admin';
