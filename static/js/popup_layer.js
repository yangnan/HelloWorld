...
var PopupLayer = new Class({
	...
	_construct:function(){                  //构造弹出层
		this.popupBlk.show();
		//如果是想默认隐藏，就用这句，否则用我下面的那句 xiezhanhui this.popupLayer.append(this.popupBlk.css({opacity:1})).appendTo($(document.body)).css({position:"absolute",'z-index':2,width:this.popupBlk.get(0).offsetWidth,height:this.popupBlk.get(0).offsetHeight});
        this.popupLayer.append(this.popupBlk.css({opacity:1})).appendTo($("#bannerset"));
		this.options.usePopupIframe?this.popupLayer.append(this.popupIframe):null;
		this.recalculatePopupIframe();
		this.popupLayer.show(); //如果默认想隐藏的话，就改成hide xiezhanhui
        //alert(this.popupLayer.css("display"));
	},
