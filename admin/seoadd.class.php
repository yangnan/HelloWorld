...
class Seoadd extends Tp{

	public function initInstance() {
		$this->saveData();
        $this->assign("link_url",$this->link_url());
		$this->display("seoadd.html");
	}
	...
	public function saveData(){
...

		{
			...
			$page = $_POST['page'];
			$pubtime=time();
					$strSQL = SqlBuilder::buildInsertSql(__PREFIX_TAB__ ."seo",array("seo_title"=>"$page_title","seo_keywords"=>"$keywords","seo_page_address"=>"$page","seo_descript"=>"$search","seo_pubtime"=>"$pubtime"));

					//执行添加函数
					if($this->conn->execute($strSQL)){
						H(lang_cp::$_TGLOBAL['uilang']['add'].lang_cp::$_TGLOBAL['uilang']['success'],"./admin.php?tg=/seolist/");
					}else{
