...
function uc_api_mysql($model, $action, $args=array()) {
	...
	if(empty($uc_controls[$model])) {
		...
		require_once UC_ROOT."/control/$model.php";
...
//xiezhanhui 加入查询所有用户功能
function uc_get_user_all($username, $isuid=0)
{
	$return = call_user_func(UC_API_FUNC, 'user', 'get_user_all', array('username'=>$username, 'isuid'=>$isuid));
	return UC_CONNECT == 'mysql' ? $return : uc_unserialize($return);
}
